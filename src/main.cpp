...
void onpublished(uint16_t packetId) {
  ...
  esp_sleep_enable_timer_wakeup(58 * 1000*1000);
  ...
  while (xQueueReceive(samples_queue, &q, portMAX_DELAY)) {
    ...
    if (Leq_samples >= SAMPLE_RATE * LEQ_PERIOD) {
     ...
     // Leq_sum_sqr = 0;
      ...
      uint16_t packetIdPub1 = mqttClient.publish(MQTT_PUB_TEST, 1, true, String(Leq_dB).c_str());          
      mqttClient.onPublish(onpublished);
    }
  }
}
